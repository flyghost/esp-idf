...
{
    core_dump_elf_version_info_t    elf_version_info;       // elf版本信息
    uint16_t                        elf_stage;              // elf阶段
    uint32_t                        elf_next_data_offset;   // elf下一个数据偏移
    uint32_t                        bad_tasks_num;          // 坏的任务数
    core_dump_task_header_t         interrupted_task;       // 中断任务
    core_dump_write_config_t *      write_cfg;              // 写入的配置
} core_dump_elf_t;
                                            ...
                                            uint32_t task_num)
{
    ...
    tot_len += data_sz; //总长度基础上加上ELF头长度
...
esp_err_t esp_core_dump_write_elf(panic_info_t *info, core_dump_write_config_t *write_cfg)
{
    ...
    static core_dump_elf_t self;        // coredump elf 句柄
    core_dump_header_t dump_hdr;        // coredump 包头
    uint32_t tcb_sz = COREDUMP_TCB_SIZE, task_num;  // tcb大小，任务个数
    int tot_len = sizeof(dump_hdr);     // 总长度
    int write_len = sizeof(dump_hdr);   // 写入长度
    ...
    // Prepare write elf 准备写入ELF
    if (write_cfg->prepare) {
        // 准备工作，更新total len ，加上校验和的长度
