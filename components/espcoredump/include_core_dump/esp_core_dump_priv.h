...
typedef struct _core_dump_write_config_t
{
    ...
    esp_core_dump_write_prepare_t       prepare;        // 准备写入
    // this function is called at the beginning of data writing
    esp_core_dump_write_start_t         start;          // 开始写入
    // this function is called when all dump data are written
    esp_core_dump_write_end_t           end;            // 写入结束
    // this function is called to write data chunk
    esp_core_dump_flash_write_data_t    write;          // 写入
...
{
    uint32_t data_len;  // data length                  coredump数据长度
    uint32_t version;   // core dump struct version     coredump版本
    uint32_t tasks_num; // number of tasks              任务数
    uint32_t tcb_sz;    // size of TCB                  TCB大小 == COREDUMP_TCB_SIZE == sizeof(StaticTask_t)
    uint32_t mem_segs_num; // number of memory segments 内存段编号
} core_dump_header_t;
...
{
    void*    tcb_addr;    // TCB address            TCB地址
    uint32_t stack_start; // stack start address    堆栈起始地址
    uint32_t stack_end;   // stack end address      堆栈结束地址
